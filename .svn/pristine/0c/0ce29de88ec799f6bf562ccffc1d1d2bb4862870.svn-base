...
// EXOAPDSignalModelBuilder
//  Builds signal models for APD signals.  See the implementation of
//  InitializeSignalModel for more details.
//______________________________________________________________________________
#include "EXOReconstruction/EXOAPDSignalModelBuilder.hh"

//______________________________________________________________________________
bool EXOAPDSignalModelBuilder::InitializationIsNeeded(const EXOSignalModel& model) const
{
...
//______________________________________________________________________________
void EXOAPDSignalModelBuilder::InitializeSignalModel(
    EXODoubleWaveform& shapeModel, 
    EXOTransferFunction& transfer) const
{
  // Initialize the signal model with an APD signal type.  This builder builds
  // the signal of an APD signal using the assumption that the initial signal
  // input into the electronics is a step function, which is a generally good
  // approximation for APD signals.  This step function is then transformed by
